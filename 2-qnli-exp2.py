...
# Load QNLI dataset
datasetqnli = load_dataset("glue", "qnli")

import random
from datasets import Dataset

train_dataset = datasetqnli["train"]
...
dataset_sanitized = dataset.map(
    lambda x: {'question': sanitize_data(x['question']), 'sentence': sanitize_data(x['sentence'])}
  )
...
for i in range(5):
    print(f"{i+1}. Question : {dataset_sanitized[i]['question']}")
    print(f"   Sentence: {dataset_sanitized[i]['sentence']}\n")
...
def train(model, dataloader, optimizer, dp_enabled=False, use_ghost=False, noise_multiplier=1.0, max_grad_norm=1.0, num_epochs=4):
...
loss_sanitized, acc_sanitized = train(model, dataloader_sanitized_half, optimizer_sanitized, dp_enabled=False, num_epochs=4)
...
# Test split
test_loader = DataLoader(val_dataset, batch_size=40)
...
privacy_engine = PrivacyEngine(
    ...
    max_grad_norm=0.1,
    target_epsilon=3,
    clipping_mode="default",  # normal clipping
)
