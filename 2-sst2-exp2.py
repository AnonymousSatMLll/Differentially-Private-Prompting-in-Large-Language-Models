...
dataset_sanitized = dataset.map(
    lambda x: {'sentence': sanitize_data(x['sentence'])}
)

print("\nHigh Contextual Redacted Data\n")
# Iterate over first 5 examples
for i in range(5):
    print(f"{i+1}. {dataset_sanitized[i]['sentence']}")
...
def get_dataloader(dataset, batch_size=42):
...
def train(model, dataloader, optimizer, dp_enabled=False, use_ghost=False, noise_multiplier=1.0, max_grad_norm=1.0, num_epochs=2):
    ...
    for epoch in range(num_epochs): # Changed to use num_epochs parameter
        ...
        # Apply clipping depending on the mode

            if use_ghost:
                for layer in model.children():
                    clip_grad_norm_(layer.parameters(), max_grad_norm)  # Ghost: Clip for each layer
...
train_loader = DataLoader(dataset, batch_size=42, shuffle=True)
...
# Test split
test_loader = DataLoader(val_dataset, batch_size=42)
